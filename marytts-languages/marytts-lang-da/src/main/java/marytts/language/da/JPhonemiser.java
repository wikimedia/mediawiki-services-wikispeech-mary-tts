...
public class JPhonemiser extends InternalModule {
	...
	protected Map<String, List<String>> secondary_lexicon;
	...
	public JPhonemiser(String componentName, MaryDataType inputType, MaryDataType outputType, String allophonesProperty,
		...
		//HB 150915 Need to allow lexiconProperty to be a list. If lexicon is *fst, load as before, if *dict, load like userlex
			...
			if (maybePronounceable(text, pos)) {
				...
				while (st.hasMoreTokens()) {
					...
					if (ph.length() == 0) { // first part
						...
						//HB 150907 Reduce accent1 and accent2 to secondary stress
						//ph.append(phon.replace('\'', ','));
						ph.append(phon.replaceAll("[\'\"]", "%"));
	...
	/**
	 * look a given text up in the secondary lexicon. part-of-speech is used in case of ambiguity.
	 * 
	 * @param text
	 * @param pos
	 * @return
	 */
	public String secondary_lexiconLookup(String text, String pos) {
		if (secondary_lexicon == null || text == null || text.length() == 0)
			return null;
		List<String> entries = secondary_lexicon.get(text);
		// If entry is not found directly, try the following changes:
		// - lowercase the word
		// - all lowercase but first uppercase
		if (entries == null) {
			text = text.toLowerCase(getLocale());
			entries = secondary_lexicon.get(text);
		}
		if (entries == null) {
			text = text.substring(0, 1).toUpperCase(getLocale()) + text.substring(1);
			entries = secondary_lexicon.get(text);
    ...
    protected Map<String, List<String>> readLexiconStream(String lexiconFilename, InputStream lexiconStream) throws IOException {
	logger.debug(String.format("Reading lexicon from '%s'", lexiconFilename));
		String line;
		Map<String, List<String>> fLexicon = new HashMap<String, List<String>>();
